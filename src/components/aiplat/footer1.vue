<template>
   <footer class="foot-fixed cm_bf"> 
    <ul class="cm_main cm_pr cm_bf cm_tc">
          ...
          <router-link tag="li" :to="vv.url" v-for="(vv,kk) in items" class="cmt2 cm_bl1ce cm_csp" v-bind:class="{'active':$route.path==vv.url,'cm_br1ce':kk==items.length-1,'cm_c4c':$route.path!=vv.url,'cm_pc_3':items.length==4,'cm_pc_4':items.length==3,'cm_pc_6':items.length==2}" v-bind:key="kk">
