<template>
  <footer class="cm_pf cm_w100 cm_z99 cm_bf" id="footerMain">
    <ul class="cm_main cm_pr cm_bf cm_tc">
      ...
      <li class="cm_pc_12 cm_mob_12 cm_hl2 cm_bb1ce cm_bf">
        <a class="cm_csp" :href="beian.url" :title="beian.title" target="_blank">
          <div class="cm_pc_12">{{beian.name}}</div>
        </a>
      </li>
      <router-link
        tag="li"
        :to="vv.url"
        v-for="(vv,kk) in items"
        class="cmt2 cm_bl1ce cm_csp"
    ...
    <ul class="cm_pf cm_blhide cm_w100 cm_pd05 cm_tc cm_be" id="seo">
      <li class="cm_pc_12 cm_lh3 cm_br02 cm_bf cm_bb1e" v-for="(v,k) in arr" v-bind:key="k">
        <a class="cm_csp" :href="v.url" :title="v.name" target="_blank">
          <div class="cm_pc_12">{{v.name}}</div>
        </a>
      </li>
...
.active {
  color: rgb(253, 20, 20);
  background: rgba(105, 104, 103, 0.14);
}
</style>